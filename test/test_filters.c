...
test_filter_add(void) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        CU_ASSERT_EQUAL(
            ip_cidr_to_in6("::ffff:192.168.1.1/120", &new_filter->iprange), 0);
...
test_filter_match(void) {
    ...
    request_t request = {.ip_type = IP_TYPE_IPV6,
...
void
test_filter_match_rejects_range_overrun(void) {
    filter_t *filter = calloc(1, sizeof(filter_t));
    CU_ASSERT_PTR_NOT_NULL_FATAL(filter);

    filter->applies_serial = 1;
    strncpy(filter->serial_id, "ttyusb0", sizeof(filter->serial_id) - 1);
    filter->slave_id = 1;
    filter->function_code = 16;
    filter->register_address_min = 10;
    filter->register_address_max = 12;
...
test_filter_match_serial(void) {
    ...
    strncpy(serial_filter->serial_id,
            "ttyusb0",
            sizeof(serial_filter->serial_id) - 1);
...
test_multiple_filters_match(void) {
    ...
    // assign values to the filter
    CU_ASSERT_EQUAL(ip_cidr_to_in6("::ffff:192.168.1.1/128", &filter1->iprange),
                    0);
    ...
    // assign values to the filter
    CU_ASSERT_EQUAL(ip_cidr_to_in6("::ffff:192.168.1.2/128", &filter2->iprange),
                    0);
    ...
    // assign values to the filter
    CU_ASSERT_EQUAL(
        ip_cidr_to_in6("::ffff:192.168.1.100/128", &filter3->iprange), 0);
    ...
    // request 1
    request_t request1 = {.ip_type = IP_TYPE_IPV4,
                          .ip = "192.168.1.1",
                          .port = "502",
                          .slave_id = 1,
                          .function = 3,
    ...
    // request 2
    request_t request2 = {.ip_type = IP_TYPE_IPV4,
                          .ip = "192.168.1.2",
                          .port = "502",
                          .slave_id = 1,
                          .function = 3,
    ...
    // request 3
    request_t request3 = {.ip_type = IP_TYPE_IPV4,
                          .ip = "192.168.1.100",
                          .port = "502",
                          .slave_id = 1,
                          .function = 3,
